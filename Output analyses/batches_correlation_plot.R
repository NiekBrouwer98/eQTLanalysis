mostsig_c1 <- filtered_ordered_length_snps_c1_batches[1:100,]
mostsig_c2 <- filtered_ordered_length_snps_c2_batches[1:100,]

different_eqtls <- setdiff(filtered_ordered_length_snps_c2_batches[,1], filtered_ordered_length_snps_c1_batches[,1])
same_eqtls <- intersect(filtered_ordered_length_snps_c1_batches[,1], filtered_ordered_length_snps_c2_batches[,1])

length(same_eqtls)
different_eqtls[1]
number <- 1
genes_most_sig_c1.1 <- SNP_list_ct1_batches[[as.character(same_eqtls[number])]][["gene"]]
genes_most_sig_c2.1 <- SNP_list_ct2_batches[[as.character(same_eqtls[number])]][["gene"]]

corresponding_genes <- genes_most_sig_c1.1[genes_most_sig_c1.1 %in% genes_most_sig_c2.1]

snp <- same_eqtls[number]
length(genes_most_sig_c1.1)
gene <- genes_most_sig_c1.1[[1]]
...
counts_list_c2 <- counts_c2_batches[[as.character(gene)]]

counts_list_c1 <- cbind(counts_c1_batches['FID'], counts_list_c1)
counts_list_c2 <- cbind(counts_c2_batches['FID'], counts_list_c2)
...
library(ggplot2)
par(mfrow=c(1,2))
p1 <- boxplot(mean_list_c1[,4]~mean_list_c1[,5], ylim=c(3,9), ylab="Expression", xlab = "Genotypes", main = paste("Cell type A. r: ",round(corr1[["estimate"]][["rho"]],2), sep = ""))
stripchart(mean_list_c1[,4]~mean_list_c1[,5], vertical = TRUE, data = mean_list_c1, 
           method = "jitter", add = TRUE, pch = 20)
p2 <- boxplot(mean_list_c2[,4]~mean_list_c2[,5], ylim=c(3,9), ylab="Expression", xlab = "Genotypes", main = paste("Cell type B. r: ",round(corr2[["estimate"]][["rho"]],2),sep = ""))
